...
- **WebSocket Message Limit**: Use `Wormholy.webSocketMessageLimit` to cap the number of messages retained per WebSocket connection. When the limit is reached, Wormholy removes the oldest messages and keeps the most recent ones. Set it to `0` to retain no messages; negative values are treated as `nil`. Changes apply to subsequently recorded messages. The default `nil` value keeps the complete message history.
